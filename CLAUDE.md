...
- `EntityIndexView`: Generic list/edit view for entity collections
- `CharacterEditView`: Full-screen character editing form (name, role, tier)
- Character cards displaying entity projections from index
...
**Entity Indices** (Parallel Storage Pattern)
- Entity indices store lightweight projections for list views (characters, locations)
- NOT TipTap documents — structured JSON with EntityIndex and EntityIndexEntry types
- Storage: `{storyId}-characters.json`, `{storyId}-locations.json`
- IPC layer: `entity:loadIndex`, `entity:saveIndex` handlers
- Zustand store manages entity index loading/caching
- Write queue protection for concurrent saves
- Future: Full CharacterDoc/LocationDoc storage for detailed entity data
