...
*Core Architecture:*
- **Two-tier storage model**: Lightweight index for lists + full entity docs for editing
- **Schema DSL**: Single source of truth for structure, validation, and UI metadata (`src/models/entities/schemas/`)
- **Type safety**: TypeScript types and Zod validation auto-generated from schemas
- **Generic components**: EntityEditView renders any entity type from schema definition
...
- **GroupDef**: Semantic grouping with labels and descriptions
- Helper functions: `defineField()`, `defineType()`, `buildZodFromSchema()`

*Storage:*
- **Entity indices**: `{storyId}-characters.json`, `{storyId}-locations.json`
  - Contains: id, name, docRef, sortOrder (minimal projections)
  - Used for: Fast list views, name lookup, sorting
- **Entity documents**: `stories/entities/{entityType}s/{entityId}.json`
  - Contains: Full structured entity data validated against schema
  - Used for: Editing, detailed views, AI context
- **Write queue protection**: Both index and docs use write queue to prevent race conditions

*IPC Layer:*
- `entity:loadIndex` / `entity:saveIndex` - Index operations
- `entity:load` / `entity:save` - Full document operations
- Client wrapper: `src/api/client.ts`
- Type definitions: `src/global.d.ts`

*UI Rendering:*
- **EntityIndexView**: Generic list/edit coordinator for any entity type
  - Switches between list mode (grid of cards) and edit mode (schema-driven form)
  - Loads full entity doc when editing
  - Updates both index (minimal) and doc (full) on save
- **EntityEditView**: Schema-driven form renderer
  - Reads DocumentTypeDef schema at runtime
...
- **Characters**: Identity, role, presence, voice, orientation, sensitivity, constraints
- **Locations**: Identity, role (schema defined, ready for use)

*LCRF Integration:*
- Entities positioned at **Layer 4: Structured Knowledge** (`storyEntities` context)
- Downstream from and informed by: Manifest (Layer 1), Story Brief (Layer 3), World (Layer 4)
- Constrains: Prose writing (Layer 5)

*Benefits:*
- **Single source of truth**: Schema defines structure, validation, and UI
- **Easy extensibility**: Add fields/entity types by updating schema only
- **Type safety**: TypeScript types derived from Zod schemas
- **Consistent validation**: Same validation rules across client and storage
- **Minimal boilerplate**: No hardcoded forms or field mappings
...
**Entities Model Integration** (LCRF Layer 4: Structured Knowledge)
- Integrated entity types (characters, locations) into document model at `storyEntities` LCRF layer
- Uses discriminated union: RichTextDocConfig vs EntityIndexDocConfig to distinguish storage patterns
- Entity indices store lightweight projections (EntityIndex/EntityIndexEntry) for efficient list views
- Complete IPC layer: `entity:loadIndex`, `entity:saveIndex` with standardized error handling
- UI implementation: EntityIndexView component with list/edit subview navigation
- Character creation system: CharacterEditView with name, role, tier fields
- Character cards display entity projections in grid view
- Entity data persisted to disk with write queue protection
- Storage: `{storyId}-characters.json`, `{storyId}-locations.json`
- Future: Full CharacterDoc/LocationDoc storage for detailed character profiles
